...
function createTables() {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      // Add client_id and client_secret columns to youtube_channels for per-channel credentials
      db.run(`ALTER TABLE youtube_channels ADD COLUMN client_id TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding client_id column to youtube_channels:', err.message);
        }
      });

      db.run(`ALTER TABLE youtube_channels ADD COLUMN client_secret TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding client_secret column to youtube_channels:', err.message);
        }
      });
