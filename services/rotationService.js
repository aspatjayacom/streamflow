...
async function startRotationStream(rotation, item) {
  try {
    ...
    if (!selectedChannel.client_id || !selectedChannel.client_secret) {
      console.error('[RotationService] YouTube API credentials not configured for this channel');
      return { success: false, error: 'YouTube API credentials not configured for this channel' };
    }

    const oauth2Client = new google.auth.OAuth2(
      selectedChannel.client_id,
      decrypt(selectedChannel.client_secret),
      getRedirectUri(user)
    );
